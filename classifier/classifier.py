...
def parse_args():
    ...
    parser.add_argument('--name_file', type=str, help='general name of data files')
    parser.add_argument('--dev_bool', type=str, default=False, help='dev split included?')
...
def load_split(data_dir, name_file, split):
    name = data_dir+name_file+'_'+split+'.csv'
    print(name)
    df = pd.read_csv(name)
    return df

def get_metrics(L, Y, label_model, tie_break_policy, metrics, split):
    scores = label_model.score(L=L, Y=Y, tie_break_policy=tie_break_policy, metrics=metrics)
...
def main(data_dir: str,
    ...
    arr_cm = confusion_matrix(Y_test, test_preds, normalize='true')
    df_cm = pd.DataFrame(data=arr_cm, columns=[i for i in range(arr_cm.shape[1])])

    #split = 'test'
    #get_metrics(L_test, Y_test, tie_break_policy, metrics, split)

    #split = 'dev'
    #if split in splits:
    #    dev_preds = label_model.predict(L_dev, tie_break_policy=tie_break_policy)
    #    get_metrics(L_dev, Y_dev, tie_break_policy, metrics, split)
    ...
    output_name = 'data_'+date_str+'labeled_'+'confusion_matrix.csv'
    filename = data_dir + output_name
    save_results(df_cm, filename)
...
if __name__ == '__main__':
    ...
    # Set dirs
    path = os.getcwd()
    main_dir = os.path.split(path)[0]+'/'+os.path.split(path)[1]
    data_dir = f"{main_dir}/data/"
    output_dir = f'{main_dir}'

    if bool(args.dev_bool):
